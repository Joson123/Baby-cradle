...
{
    gizwitsGetNTP();
    g_year = gizwitsProtocol.TimeNTP.year;
    g_month = gizwitsProtocol.TimeNTP.month;
    g_day = gizwitsProtocol.TimeNTP.day;
    g_hour = gizwitsProtocol.TimeNTP.hour;
    g_minute = gizwitsProtocol.TimeNTP.minute;
    printf("\r\r\nzht get net time: [%d/%d/%d][%d:%d]\r\r\n", g_year, g_month, g_day, g_hour, g_minute);
}
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagtemp_check)
    {
        info->event[info->num] = EVENT_temp_check;
        info->num++;
        dataPoints->valuetemp_check = gizStandardDecompressionValue(temp_check_BYTEOFFSET,temp_check_BITOFFSET,temp_check_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagswing_state)
    {
        info->event[info->num] = EVENT_swing_state;
        info->num++;
        dataPoints->valueswing_state = gizStandardDecompressionValue(swing_state_BYTEOFFSET,swing_state_BITOFFSET,swing_state_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagWarm_milk)
    {
        info->event[info->num] = EVENT_Warm_milk;
        info->num++;
        dataPoints->valueWarm_milk = gizStandardDecompressionValue(Warm_milk_BYTEOFFSET,Warm_milk_BITOFFSET,Warm_milk_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flaghumidification)
    {
        info->event[info->num] = EVENT_humidification;
        info->num++;
        dataPoints->valuehumidification = gizStandardDecompressionValue(humidification_BYTEOFFSET,humidification_BITOFFSET,humidification_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagweight_on_of)
    {
        info->event[info->num] = EVENT_weight_on_of;
        info->num++;
        dataPoints->valueweight_on_of = gizStandardDecompressionValue(weight_on_of_BYTEOFFSET,weight_on_of_BITOFFSET,weight_on_of_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagpurifier)
    {
        info->event[info->num] = EVENT_purifier;
        info->num++;
        dataPoints->valuepurifier = gizStandardDecompressionValue(purifier_BYTEOFFSET,purifier_BITOFFSET,purifier_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagwake_up_testing_switch)
    {
        info->event[info->num] = EVENT_wake_up_testing_switch;
        info->num++;
        dataPoints->valuewake_up_testing_switch = gizStandardDecompressionValue(wake_up_testing_switch_BYTEOFFSET,wake_up_testing_switch_BITOFFSET,wake_up_testing_switch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagcover_the_quilt_switch)
    {
        info->event[info->num] = EVENT_cover_the_quilt_switch;
        info->num++;
        dataPoints->valuecover_the_quilt_switch = gizStandardDecompressionValue(cover_the_quilt_switch_BYTEOFFSET,cover_the_quilt_switch_BITOFFSET,cover_the_quilt_switch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagplay_the_music)
    {
        info->event[info->num] = EVENT_play_the_music;
        info->num++;
        dataPoints->valueplay_the_music = gizStandardDecompressionValue(play_the_music_BYTEOFFSET,play_the_music_BITOFFSET,play_the_music_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuetemp_check != cur->valuetemp_check)
    {
        GIZWITS_LOG("valuetemp_check Changed\n");
        ret = 1;
    }
    if(last->valueswing_state != cur->valueswing_state)
    {
        GIZWITS_LOG("valueswing_state Changed\n");
        ret = 1;
    }
    if(last->valueWarm_milk != cur->valueWarm_milk)
    {
        GIZWITS_LOG("valueWarm_milk Changed\n");
        ret = 1;
    }
    if(last->valuehumidification != cur->valuehumidification)
    {
        GIZWITS_LOG("valuehumidification Changed\n");
        ret = 1;
    }
    if(last->valueweight_on_of != cur->valueweight_on_of)
    {
        GIZWITS_LOG("valueweight_on_of Changed\n");
        ret = 1;
    }
    if(last->valuepurifier != cur->valuepurifier)
    {
        GIZWITS_LOG("valuepurifier Changed\n");
        ret = 1;
    }
    if(last->valuewake_up_testing_switch != cur->valuewake_up_testing_switch)
    {
        GIZWITS_LOG("valuewake_up_testing_switch Changed\n");
        ret = 1;
    }
    if(last->valuecover_the_quilt_switch != cur->valuecover_the_quilt_switch)
    {
        GIZWITS_LOG("valuecover_the_quilt_switch Changed\n");
        ret = 1;
    }
    if(last->valueplay_the_music != cur->valueplay_the_music)
    {
        GIZWITS_LOG("valueplay_the_music Changed\n");
        ret = 1;
    }
    if(last->valuerain_water != cur->valuerain_water)
    {
        GIZWITS_LOG("valuerain_water Changed\n");
        ret = 1;
    }
    if(last->valuecover_the_quilt != cur->valuecover_the_quilt)
    {
        GIZWITS_LOG("valuecover_the_quilt Changed\n");
        ret = 1;
    }
    if(last->valueon_the_bed != cur->valueon_the_bed)
    {
        GIZWITS_LOG("valueon_the_bed Changed\n");
        ret = 1;
    }
    if(last->valuewake_up_testing != cur->valuewake_up_testing)
    {
        GIZWITS_LOG("valuewake_up_testing Changed\n");
        ret = 1;
    }

    if(last->valuedht11_tempe != cur->valuedht11_tempe)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedht11_tempe Changed\n");
            ret = 1;
        }
    }
    if(last->valuedht11_humidity != cur->valuedht11_humidity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedht11_humidity Changed\n");
            ret = 1;
        }
    }
    ...
    if(last->valuemlx90614 != cur->valuemlx90614)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuemlx90614 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(temp_check_BYTEOFFSET,temp_check_BITOFFSET,temp_check_LEN,(uint8_t *)devStatusPtr,dataPoints->valuetemp_check);
    gizStandardCompressValue(swing_state_BYTEOFFSET,swing_state_BITOFFSET,swing_state_LEN,(uint8_t *)devStatusPtr,dataPoints->valueswing_state);
    gizStandardCompressValue(Warm_milk_BYTEOFFSET,Warm_milk_BITOFFSET,Warm_milk_LEN,(uint8_t *)devStatusPtr,dataPoints->valueWarm_milk);
    gizStandardCompressValue(humidification_BYTEOFFSET,humidification_BITOFFSET,humidification_LEN,(uint8_t *)devStatusPtr,dataPoints->valuehumidification);
    gizStandardCompressValue(weight_on_of_BYTEOFFSET,weight_on_of_BITOFFSET,weight_on_of_LEN,(uint8_t *)devStatusPtr,dataPoints->valueweight_on_of);
    gizStandardCompressValue(purifier_BYTEOFFSET,purifier_BITOFFSET,purifier_LEN,(uint8_t *)devStatusPtr,dataPoints->valuepurifier);
    gizStandardCompressValue(wake_up_testing_switch_BYTEOFFSET,wake_up_testing_switch_BITOFFSET,wake_up_testing_switch_LEN,(uint8_t *)devStatusPtr,dataPoints->valuewake_up_testing_switch);
    gizStandardCompressValue(cover_the_quilt_switch_BYTEOFFSET,cover_the_quilt_switch_BITOFFSET,cover_the_quilt_switch_LEN,(uint8_t *)devStatusPtr,dataPoints->valuecover_the_quilt_switch);
    if(dataPoints->valueplay_the_music >= play_the_music_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueplay_the_music Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(play_the_music_BYTEOFFSET,play_the_music_BITOFFSET,play_the_music_LEN,(uint8_t *)devStatusPtr,dataPoints->valueplay_the_music);  
    }
    if(dataPoints->valuerain_water >= rain_water_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuerain_water Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(rain_water_BYTEOFFSET,rain_water_BITOFFSET,rain_water_LEN,(uint8_t *)devStatusPtr,dataPoints->valuerain_water);  
    }
    if(dataPoints->valuecover_the_quilt >= cover_the_quilt_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuecover_the_quilt Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(cover_the_quilt_BYTEOFFSET,cover_the_quilt_BITOFFSET,cover_the_quilt_LEN,(uint8_t *)devStatusPtr,dataPoints->valuecover_the_quilt);  
    }
    if(dataPoints->valueon_the_bed >= on_the_bed_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueon_the_bed Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(on_the_bed_BYTEOFFSET,on_the_bed_BITOFFSET,on_the_bed_LEN,(uint8_t *)devStatusPtr,dataPoints->valueon_the_bed);  
    }
    if(dataPoints->valuewake_up_testing >= wake_up_testing_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuewake_up_testing Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(wake_up_testing_BYTEOFFSET,wake_up_testing_BITOFFSET,wake_up_testing_LEN,(uint8_t *)devStatusPtr,dataPoints->valuewake_up_testing);  
    }
    ...
    devStatusPtr->valuedht11_tempe = exchangeBytes(gizY2XFloat(dht11_tempe_RATIO,  dht11_tempe_ADDITION, dataPoints->valuedht11_tempe));     
    devStatusPtr->valuedht11_humidity = exchangeBytes(gizY2XFloat(dht11_humidity_RATIO,  dht11_humidity_ADDITION, dataPoints->valuedht11_humidity));     
    devStatusPtr->valuemq_7 = exchangeBytes(gizY2XFloat(mq_7_RATIO,  mq_7_ADDITION, dataPoints->valuemq_7));     
    devStatusPtr->valuemlx90614 = exchangeBytes(gizY2XFloat(mlx90614_RATIO,  mlx90614_ADDITION, dataPoints->valuemlx90614));     

    devStatusPtr->valuelongitude = exchangeWord(gizY2XFloat(longitude_RATIO,  longitude_ADDITION, dataPoints->valuelongitude)); 
    devStatusPtr->valuelatitude = exchangeWord(gizY2XFloat(latitude_RATIO,  latitude_ADDITION, dataPoints->valuelatitude)); 
